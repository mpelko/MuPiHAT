...
@dataclass
class ChargerData:
    """Dataclass representing the charger status and measurements."""
...
class bq25792:
    ...
    to_dict()
        Returns a dictionary with key metrics such as voltage, current, temperature, and charger status.
    to_json_registers()
        ...
        try:
            ...
            for bt in config["mupihat"]["battery_types"]:
                if bt["name"] == selected_battery_name:
                    ...
                    logging.info(
                        "Battery configuration loaded from JSON: %s",
                        self.battery_conf_file,
                    )
                    break
            return 0
        except Exception as _error:
            logging.error(
                "battery_conf_load from JSON failed, use standard configuration, %s",
                str(_error),
            )
            if self._exit_on_error:
                sys.exit(1)
    ...
    def battery_soc(self):
        ...
        Bat_SOC = ""
        Bat_Stat = ""
        v_100, v_75, v_50, v_25, v_0 = (
            self.battery_conf["v_100"],
            self.battery_conf["v_75"],
            self.battery_conf["v_50"],
            self.battery_conf["v_25"],
            self.battery_conf["v_0"],
        )
        th_warning, th_shutdown = (
            self.battery_conf["th_warning"],
            self.battery_conf["th_shutdown"],
        )
        ...
        if VBat > th_warning:
            Bat_Stat = "OK"
        elif (VBat < th_warning) & (VBat > th_shutdown):
            Bat_Stat = "LOW"
        elif VBat < th_shutdown:
            Bat_Stat = "SHUTDOWN"
    ...
    class REG0E_Timer_Control(BQ25795_REGISTER):
        ...
        def get(self):
            self._value = (
                (self.TOPOFF_TMR << 6)
                | (self.EN_TRICHG_TMR << 5)
                | (self.EN_PRECHG_TMR << 4)
                | (self.EN_CHG_TMR << 3)
                | (self.CHG_TMR << 1)
                | (self.TMR2X_EN << 0)
            )
    ...
    class REG0F_Charger_Control_0(BQ25795_REGISTER):
        ...
        def get(self):
            self._value = (
                (self.EN_AUTO_IBATDIS << 7)
                | (self.FORCE_IBATDIS << 6)
                | (self.EN_CHG << 5)
                | (self.EN_ICO << 4)
                | (self.FORCE_ICO << 3)
                | (self.EN_HIZ << 2)
    ...
    class REG13_Charger_Control_4(BQ25795_REGISTER):
        ...
        def get(self):
            self._value = (
                ...
                | (self.PWM_FREQ << 5)
                | (self.DIS_STAT << 4)
                | (self.DIS_VSYS_SHORT << 3)
                | (self.DIS_VOTG_UVP << 2)
                | (self.FORCE_VINDPM_DET << 1)
                | (self.EN_IBUS_OCP)
            )
            return (
                self._value,
    ...
    class REG17_NTC_Control_0(BQ25795_REGISTER):
        ...
        def get_JEITA_VSET_str(self):
            if self.JEITA_VSET == 0x0:
                return "Charge Suspend"
            elif self.JEITA_VSET == 0x1:
                return "Set VREG to VREG-800mV"
            elif self.JEITA_VSET == 0x2:
                return "Set VREG to VREG-600mV"
            elif self.JEITA_VSET == 0x3:
                return "Set VREG to VREG-400mV (default)"
            elif self.JEITA_VSET == 0x4:
                return "Set VREG to VREG-300mV"
            elif self.JEITA_VSET == 0x5:
                return "Set VREG to VREG-200mV"
            elif self.JEITA_VSET == 0x6:
                return "Set VREG to VREG-100mV"
            elif self.JEITA_VSET == 0x7:
                return "VREG unchanged"
            else:
                return "unknown"
        ...
        def get_JEITA_ISETH_str(self):
            if self.JEITA_ISETH == 0x0:
                return "Charge Suspend"
            elif self.JEITA_ISETH == 0x1:
                return "Set ICHG to 20%* ICHG"
            elif self.JEITA_ISETH == 0x2:
                return "Set ICHG to 40%* ICHG"
            elif self.JEITA_ISETH == 0x3:
                return "ICHG unchanged (default)"
            else:
                return "unknown"
        ...
        def get_JEITA_ISETC_str(self):
            if self.JEITA_ISETC == 0x0:
                return "Charge Suspend"
            elif self.JEITA_ISETC == 0x1:
                return "Set ICHG to 20%* ICHG (default)"
            elif self.JEITA_ISETC == 0x2:
                return "Set ICHG to 40%* ICHG"
            elif self.JEITA_ISETC == 0x3:
                return "ICHG unchanged"
            else:
                return "unknown"
    ...
    class REG18_NTC_Control_1(BQ25795_REGISTER):
        ...
        def get_TS_COOL_threshold(self):
            if self.TS_COOL == 0x0:
                return "5°C"
            elif self.TS_COOL == 0x1:
                return "10°C"
            elif self.TS_COOL == 0x2:
                return "15°C"
            elif self.TS_COOL == 0x3:
                return "20°C"
            else:
                return "unknown"
        ...
        def get_TS_WARM_threshold(self):
            if self.TS_WARM == 0x0:
                return "40°C"
            elif self.TS_WARM == 0x1:
                return "45°C"
            elif self.TS_WARM == 0x2:
                return "50°C"
            elif self.TS_WARM == 0x3:
                return "55°C"
            else:
                return "unknown"
        ...
        def get_TS_IGNORE_status(self):
            if self.TS_IGNORE == 0x0:
                return "NOT ignore (Default)"
            elif self.TS_IGNORE == 0x1:
                return "Ignore"
            else:
                return "unknown"
    ...
    class REG1B_Charger_Status_0(BQ25795_REGISTER):
        ...
        def get(self):
            self._value = (
                ...
                | (self.POORSRC_STAT << 4)
                | (self.PG_STAT << 3)
                | (self.AC2_PRESENT_STAT << 2)
                | (self.AC1_PRESENT_STAT << 1)
                | self.VBUS_PRESENT_STAT
            )
            return (
                self._value,
                self.IINDPM_STAT,
                ...
                self.POORSRC_STAT,
                self.PG_STAT,
                self.AC2_PRESENT_STAT,
                self.AC1_PRESENT_STAT,
                self.VBUS_PRESENT_STAT,
            )
    ...
    class REG1C_Charger_Status_1(BQ25795_REGISTER):
        ...
        def get_VBUS_STAT_string(self):
            ...
            Ch: Reserved Dh: Reserved Eh: Reserved Fh: Reserved
            """
            if self.VBUS_STAT == 0x0:
                return "No Input or BHOT or BCOLD in OTG mode"
            elif self.VBUS_STAT == 0x1:
                return "USB SDP (500mA)"
            elif self.VBUS_STAT == 0x2:
                return "USB CDP (1.5A)"
            elif self.VBUS_STAT == 0x3:
                return "USB DCP (3.25A)"
            elif self.VBUS_STAT == 0x4:
                return "Adjustable High Voltage DCP (HVDCP) (1.5A)"
            elif self.VBUS_STAT == 0x5:
                return "Unknown adaptor (3A)"
            elif self.VBUS_STAT == 0x6:
                return "Non-Standard Adapter (1A/2A/2.1A/2.4A)"
            elif self.VBUS_STAT == 0x7:
                return "In OTG mode"
            elif self.VBUS_STAT == 0x8:
                return "Not qualified adaptor"
            elif self.VBUS_STAT == 0x9:
                return "Reserved"
            elif self.VBUS_STAT == 0xA:
                return "Reserved"
            elif self.VBUS_STAT == 0xB:
                return "Device directly powered from VBUS"
            elif self.VBUS_STAT == 0xC:
                return "Reserved"
            elif self.VBUS_STAT == 0xD:
                return "Reserved"
            elif self.VBUS_STAT == 0xE:
                return "Reserved"
            elif self.VBUS_STAT == 0xF:
                return "Reserved"
            else:
                return "Reserved"
    ...
    class REG1D_Charger_Status_2(BQ25795_REGISTER):
        ...
        def get_thermal_regulation_status(self):
            ...
            if self.TREG_STAT == 0:
                return "Normal"
            elif self.TREG_STAT == 1:
                return "Device in thermal regulation"
            else:
                return "unknown"
    ...
    class REG1E_Charger_Status_3(BQ25795_REGISTER):
        ...
        def get(self):
            return (
                ...
                self.VSYS_STAT,
                self.CHG_TMR_STAT,
                self.TRICHG_TMR_STAT,
                self.PRECHG_TMR_STAT,
            )
    ...
    class REG1F_Charger_Status_4(BQ25795_REGISTER):
        ...
        def get_VBATOTG_LOW_STAT_string(self):
            ...
            1h = The battery volage is too low to enable the OTG operation
            """
            if self.VBATOTG_LOW_STAT == 0:
                return "The battery voltage is high enough to enable the OTG operation"
            elif self.VBATOTG_LOW_STAT == 1:
                return "The battery volage is too low to enable the OTG operation"
            else:
                return "unknown"
        ...
        def get_TS_COLD_STAT_string(self):
            ...
            1h = TS status is in cold range
            """
            if self.TS_COLD_STAT == 0:
                return "TS status is NOT in cold range"
            elif self.TS_COLD_STAT == 1:
                return "TS status is in cold range"
            else:
                return "unknown"
        ...
        def get_TS_COOL_STAT_string(self):
            ...
            1h = TS status is in cool range
            """
            if self.TS_COOL_STAT == 0:
                return "TS status is NOT in cool range"
            elif self.TS_COOL_STAT == 1:
                return "TS status is in cool range"
            else:
                return "unknown"
        ...
        def get_TS_WARM_STAT_string(self):
            ...
            1h = TS status is in warm range
            """
            if self.TS_WARM_STAT == 0:
                return "TS status is NOT in warm range"
            elif self.TS_WARM_STAT == 1:
                return "TS status is in warm range"
            else:
                return "unknown"
        ...
        def get_TS_HOT_STAT_string(self):
            ...
            1h = TS status is in hot range
            """
            if self.TS_HOT_STAT == 0:
                return "TS status is NOT in hot range"
            elif self.TS_HOT_STAT == 1:
                return "TS status is in hot range"
            else:
                return "unknown"
    ...
    class REG20_FAULT_Status_0(BQ25795_REGISTER):
        ...
        def get_VBUS_OVP_STAT_string(self):
            ...
            1h = Device in over voltage protection
            """
            if self.VBUS_OVP_STAT == 0:
                return "Normal"
            elif self.VBUS_OVP_STAT == 1:
                return "Device in over voltage protection"
            else:
                return "unknown"
        ...
        def get_VBAT_OVP_STAT_string(self):
            ...
            1h = Device in over voltage protection
            """
            if self.VBAT_OVP_STAT == 0:
                return "Normal"
            elif self.VBAT_OVP_STAT == 1:
                return "Device in over voltage protection"
            else:
                return "unknown"
        ...
        def get_IBAT_REG_STAT_string(self):
            ...
            1h = Device in battery discharging current regulation
            """
            if self.IBAT_REG_STAT == 0:
                return "Normal"
            elif self.IBAT_REG_STAT == 1:
                return "Device in battery discharging current regulation"
            else:
                return "unknown"
        ...
        def get_IBUS_OCP_STAT_string(self):
            ...
            1h = Device in over current protection
            """
            if self.IBUS_OCP_STAT == 0:
                return "Normal"
            elif self.IBUS_OCP_STAT == 1:
                return "Device in over current protection"
            else:
                return "unknown"
        ...
        def get_IBAT_OCP_STAT_string(self):
            ...
            1h = Device in over current protection
            """
            if self.IBAT_OCP_STAT == 0:
                return "Normal"
            elif self.IBAT_OCP_STAT == 1:
                return "Device in over current protection"
            else:
                return "unknown"
        ...
        def get_CONV_OCP_STAT_string(self):
            ...
            1h = Converter in over current protection
            """
            if self.CONV_OCP_STAT == 0:
                return "Normal"
            elif self.CONV_OCP_STAT == 1:
                return "Converter in over current protection"
            else:
                return "unknown"
        ...
        def get_VAC2_OVP_STAT_string(self):
            ...
            1h = Device in over voltage protection
            """
            if self.VAC2_OVP_STAT == 0:
                return "Normal"
            elif self.VAC2_OVP_STAT == 1:
                return "Device in over voltage protection"
            else:
                return "unknown"
        ...
        def get_VAC1_OVP_STAT_string(self):
            ...
            1h = Device in over voltage protection
            """
            if self.VAC1_OVP_STAT == 0:
                return "Normal"
            elif self.VAC1_OVP_STAT == 1:
                return "Device in over voltage protection"
            else:
                return "unknown"
    ...
    class REG21_FAULT_Status_1(BQ25795_REGISTER):
        ...
        def get_VSYS_SHORT_STAT_string(self):
            ...
            1h = Device in SYS short circuit protection
            """
            if self.VSYS_SHORT_STAT == 0:
                return "Normal"
            elif self.VSYS_SHORT_STAT == 1:
                return "Device in SYS short circuit protection"
            else:
                return "unknown"
        ...
        def get_VSYS_OVP_STAT_string(self):
            ...
            1h = Device in over voltage protection
            """
            if self.VSYS_OVP_STAT == 0:
                return "Normal"
            elif self.VSYS_OVP_STAT == 1:
                return "Device in over voltage protection"
            else:
                return "unknown"
        ...
        def get_OTG_OVP_STAT_string(self):
            ...
            1h = Device in OTG over voltage
            """
            if self.OTG_OVP_STAT == 0:
                return "Normal"
            elif self.OTG_OVP_STAT == 1:
                return "Device in OTG over voltage"
            else:
                return "unknown"
        ...
        def get_OTG_UVP_STAT_string(self):
            ...
            1h = Device in OTG under voltage protection
            """
            if self.OTG_UVP_STAT == 0:
                return "Normal"
            elif self.OTG_UVP_STAT == 1:
                return "Device in OTG under voltage protection"
            else:
                return "unknown"
        ...
        def get_TSHUT_STAT_string(self):
            ...
            1h = Device in thermal shutdown protection
            """
            if self.TSHUT_STAT == 0:
                return "Normal"
            elif self.TSHUT_STAT == 1:
                return "Device in thermal shutdown protection"
            else:
                return "unknown"
    ...
    class REG22_Charger_Flag_0(BQ25795_REGISTER):
        ...
        def get(self):
            return (
                ...
                self.POORSRC_FLAG,
                self.PG_FLAG,
                self.VAC2_PRESENT_FLAG,
                self.VAC1_PRESENT_FLAG,
                self.VBUS_PRESENT_FLAG,
            )
        ...
        def get_IINDPM_FLAG_string(self):
            ...
            1h = IINDPM / IOTG signal rising edge detected
            """
            if self.IINDPM_FLAG == 0:
                return "Normal"
            elif self.IINDPM_FLAG == 1:
                return "IINDPM / IOTG signal rising edge detected"
            else:
                return "unknown"
        ...
        def get_VINDPM_FLAG_string(self):
            ...
            1h = VINDPM signal rising edge detected
            """
            if self.VINDPM_FLAG == 0:
                return "Normal"
            elif self.VINDPM_FLAG == 1:
                return "VINDPM signal rising edge detected"
            else:
                return "unknown"
        ...
        def get_WD_FLAG_string(self):
            ...
            if self.WD_FLAG == 0:
                ...
                return "WD timer signal rising edge detected"
            else:
                return "unknown"
        ...
        def get_POORSRC_FLAG_string(self):
            ...
            1h = Poor source status rising edge detected
            """
            if self.POORSRC_FLAG == 0:
                return "Normal"
            elif self.POORSRC_FLAG == 1:
                return "Poor source status rising edge detected"
            else:
                return "unknown"
        ...
        def get_VAC2_PRESENT_FLAG_string(self):
            ...
            1h = VAC2 present status changed
            """
            if self.VAC2_PRESENT_FLAG == 0:
                return "Normal"
            elif self.VAC2_PRESENT_FLAG == 1:
                return "VAC2 present status changed"
            else:
                return "unknown"
        ...
        def get_VAC1_PRESENT_FLAG_string(self):
            ...
            1h = VAC1 present status changed
            """
            if self.VAC1_PRESENT_FLAG == 0:
                return "Normal"
            elif self.VAC1_PRESENT_FLAG == 1:
                return "VAC1 present status changed"
            else:
                return "unknown"
        ...
        def get_VBUS_PRESENT_FLAG_string(self):
            ...
            1h = VBUS present status changed
            """
            if self.VBUS_PRESENT_FLAG == 0:
                return "Normal"
            elif self.VBUS_PRESENT_FLAG == 1:
                return "VBUS present status changed"
            else:
                return "unknown"
    ...
    class REG23_Charger_Flag_1(BQ25795_REGISTER):
        ...
        def get(self):
            return (
                ...
                self.VBUS_FLAG,
                self.TREG_FLAG,
                self.VBAT_PRESENT_FLAG,
                self.BC1_2_DONE_FLAG,
            )
        ...
        def get_ICO_FLAG_string(self):
            ...
            if self.ICO_FLAG == 0:
                return "Normal"
            elif self.ICO_FLAG == 1:
                return "ICO status changed"
            else:
                return "unknown"
        ...
        def get_VBUS_FLAG_string(self):
            ...
            if self.VBUS_FLAG == 0:
                return "Normal"
            elif self.VBUS_FLAG == 1:
                return "VBUS status changed"
            else:
                return "unknown"
        ...
        def get_TREG_FLAG_string(self):
            ...
            if self.TREG_FLAG == 0:
                return "Normal"
            elif self.TREG_FLAG == 1:
                return "TREG signal rising threshold detected"
            else:
                return "unknown"
        ...
        def get_VBAT_PRESENT_FLAG_string(self):
            ...
            1h = VBAT present status changed
            """
            if self.VBAT_PRESENT_FLAG == 0:
                return "Normal"
            elif self.VBAT_PRESENT_FLAG == 1:
                return "VBAT present status changed"
            else:
                return "unknown"
        ...
        def get_BC1_2_DONE_FLAG_string(self):
            ...
            1h = BC1.2 detection status changed
            """
            if self.BC1_2_DONE_FLAG == 0:
                return "Normal"
            elif self.BC1_2_DONE_FLAG == 1:
                return "BC1.2 detection status changed"
            else:
                return "unknown"
    ...
    class REG24_Charger_Flag_2(BQ25795_REGISTER):
        ...
        def get(self):
            return (
                ...
                self.VSYS_FLAG,
                self.CHG_TMR_FLAG,
                self.TRICHG_TMR_FLAG,
                self.PRECHG_TMR_FLAG,
                self.TOPOFF_TMR_FLAG,
            )
        ...
        def get_DPDM_DONE_FLAG_string(self):
            ...
            1h =  D+/D- detection is completed
            """
            if self.DPDM_DONE_FLAG == 0:
                return "D+/D- detection is NOT started or still ongoing"
            elif self.DPDM_DONE_FLAG == 1:
                return "D+/D- detection is completed"
            else:
                return "unknown"
        ...
        def get_ADC_DONE_FLAG_string(self):
            ...
            1h = Conversion completed
            """
            if self.ADC_DONE_FLAG == 0:
                return "Conversion NOT completed"
            elif self.ADC_DONE_FLAG == 1:
        ...
        def get_VSYS_FLAG_string(self):
            ...
            if self.VSYS_FLAG == 0:
                return "Normal"
            elif self.VSYS_FLAG == 1:
                return "Entered or existed VSYSMIN regulation"
            else:
                return "unknown"
        ...
        def get_CHG_TMR_FLAG_string(self):
            ...
            1h = Fast charge timer expired rising edge detected
            """
            if self.CHG_TMR_FLAG == 0:
                return "Normal"
            elif self.CHG_TMR_FLAG == 1:
                return "Fast charge timer expired rising edge detected"
            else:
                return "unknown"
        ...
        def get_TRICHG_TMR_FLAG_string(self):
            ...
            1h = Trickle charger timer expired rising edge detected
            """
            if self.TRICHG_TMR_FLAG == 0:
                return "Normal"
            elif self.TRICHG_TMR_FLAG == 1:
                return "Trickle charger timer expired rising edge detected"
            else:
                return "unknown"
        ...
        def get_PRECHG_TMR_FLAG_string(self):
            ...
            1h = Pre-charge timer expired rising edge detected
            """
            if self.PRECHG_TMR_FLAG == 0:
                return "Normal"
            elif self.PRECHG_TMR_FLAG == 1:
                return "Pre-charge timer expired rising edge detected"
            else:
                return "unknown"
        ...
        def get_TOPOFF_TMR_FLAG_string(self):
            ...
            1h = Top off timer expired rising edge detected
            """
            if self.TOPOFF_TMR_FLAG == 0:
                return "Normal"
            elif self.TOPOFF_TMR_FLAG == 1:
                return "Top off timer expired rising edge detected"
            else:
                return "unknown"
    ...
    class REG25_Charger_Flag_3(BQ25795_REGISTER):
        ...
        def get_VBATOTG_LOW_FLAG_string(self):
            ...
            1h = VBAT falls below the threshold to enable the OTG mode
            """
            if self.VBATOTG_LOW_FLAG == 0:
                return "Normal"
            elif self.VBATOTG_LOW_FLAG == 1:
                return "VBAT falls below the threshold to enable the OTG mode"
            else:
                return "unknown"
        ...
        def get_TS_COLD_FLAG_string(self):
            ...
            1h = TS across cold temperature (T1) is detected
            """
            if self.TS_COLD_FLAG == 0:
                return "Normal"
            elif self.TS_COLD_FLAG == 1:
                return "TS across cold temperature (T1) is detected"
            else:
                return "unknown"
        ...
        def get_TS_COOL_FLAG_string(self):
            ...
            1h = TS across cool temperature (T2) is detected
            """
            if self.TS_COOL_FLAG == 0:
                return "Normal"
            elif self.TS_COOL_FLAG == 1:
                return "TS across cool temperature (T2) is detected"
            else:
                return "unknown"
        ...
        def get_TS_WARM_FLAG_string(self):
            ...
            """
            if self.TS_WARM_FLAG == 0:
                return "Normal"
            elif self.TS_WARM_FLAG == 1:
                return "TS across warm temperature (T3) is detected"
            else:
                return "unknown"
        ...
        def get_TS_HOT_FLAG_string(self):
            ...
            1h = TS across hot temperature (T5) is detected
            """
            if self.TS_HOT_FLAG == 0:
                return "Normal"
            elif self.TS_HOT_FLAG == 1:
                return "TS across hot temperature (T5) is detected"
            else:
                return "unknown"
    ...
    class REG26_FAULT_Flag_0(BQ25795_REGISTER):
        ...
        def get_IBAT_REG_FLAG_string(self):
            ...
            1h = Enter or exit IBAT regulation
            """
            if self.IBAT_REG_FLAG == 0:
                return "Normal"
            elif self.IBAT_REG_FLAG == 1:
                return "Enter or exit IBAT regulation"
            else:
                return "unknown"
        ...
        def get_VBUS_OVP_FLAG_string(self):
            ...
            1h = VBUS over voltage protection triggered
            """
            if self.VBUS_OVP_FLAG == 0:
                return "Normal"
            elif self.VBUS_OVP_FLAG == 1:
                return "VBUS over voltage protection triggered"
            else:
                return "unknown"
        ...
        def get_VBAT_OVP_FLAG_string(self):
            ...
            1h = VBAT over voltage protection triggered
            """
            if self.VBAT_OVP_FLAG == 0:
                return "Normal"
            elif self.VBAT_OVP_FLAG == 1:
                return "VBAT over voltage protection triggered"
            else:
                return "unknown"
        ...
        def get_IBUS_OCP_FLAG_string(self):
            ...
            1h = IBUS over current protection triggered
            """
            if self.IBUS_OCP_FLAG == 0:
                return "Normal"
            elif self.IBUS_OCP_FLAG == 1:
                return "IBUS over current protection triggered"
            else:
                return "unknown"
        ...
        def get_IBAT_OCP_FLAG_string(self):
            ...
            1h = IBAT over current protection triggered
            """
            if self.IBAT_OCP_FLAG == 0:
                return "Normal"
            elif self.IBAT_OCP_FLAG == 1:
                return "IBAT over current protection triggered"
            else:
                return "unknown"
        ...
        def get_CONV_OCP_FLAG_string(self):
            ...
            1h = Converter over current protection triggered
            """
            if self.CONV_OCP_FLAG == 0:
                return "Normal"
            elif self.CONV_OCP_FLAG == 1:
                return "Converter over current protection triggered"
            else:
                return "unknown"
        ...
        def get_VAC2_OVP_FLAG_string(self):
            ...
            1h = VAC2 over voltage protection triggered
            """
            if self.VAC2_OVP_FLAG == 0:
                return "Normal"
            elif self.VAC2_OVP_FLAG == 1:
                return "VAC2 over voltage protection triggered"
            else:
                return "unknown"
        ...
        def get_VAC1_OVP_FLAG_string(self):
            ...
            1h = VAC1 over voltage protection triggered
            """
            if self.VAC1_OVP_FLAG == 0:
                return "Normal"
            elif self.VAC1_OVP_FLAG == 1:
                return "VAC1 over voltage protection triggered"
            else:
                return "unknown"
    ...
    class REG27_FAULT_Flag_1(BQ25795_REGISTER):
        ...
        def get_VSYS_SHORT_FLAG_string(self):
            ...
            1h = Stop switching due to system short
            """
            if self.VSYS_SHORT_FLAG == 0:
                return "Normal"
            elif self.VSYS_SHORT_FLAG == 1:
                return "Stop switching due to system short"
            else:
                return "unknown"
        ...
        def get_VSYS_OVP_FLAG_string(self):
            ...
            1h = Stop switching due to system over-voltage
            """
            if self.VSYS_OVP_FLAG == 0:
                return "Normal"
            elif self.VSYS_OVP_FLAG == 1:
                return "Stop switching due to system over-voltage"
            else:
                return "unknown"
        ...
        def get_OTG_OVP_FLAG_string(self):
            ...
            1h = Stop OTG due to VBUS over voltage
            """
            if self.OTG_OVP_FLAG == 0:
                return "Normal"
            elif self.OTG_OVP_FLAG == 1:
                return "Stop OTG due to VBUS over voltage"
            else:
                return "unknown"
        ...
        def get_OTG_UVP_FLAG_string(self):
            ...
            1h = Stop OTG due to VBUS under voltage
            """
            if self.OTG_UVP_FLAG == 0:
                return "Normal"
            elif self.OTG_UVP_FLAG == 1:
                return "Stop OTG due to VBUS under voltage"
            else:
                return "unknown"
        ...
        def get_TSHUT_FLAG_string(self):
            ...
            1h = TS shutdown signal rising threshold detected
            """
            if self.TSHUT_FLAG == 0:
                return "Normal"
            elif self.TSHUT_FLAG == 1:
                return "TS shutdown signal rising threshold detected"
            else:
                return "unknown"
    ...
    class REG28_Charger_Mask_0(BQ25795_REGISTER):
        ...
        def get(self):
            ...
            self._value = (
                ...
                | (self.POORSRC_MASK << 4)
                | (self.PG_MASK << 3)
                | (self.VAC2_PRESENT_MASK << 2)
                | (self.VAC1_PRESENT_MASK << 1)
                | (self.VBUS_PRESENT_MASK << 0)
            )
            return (
                self._value,
                self.IINDPM_MASK,
                ...
                self.POORSRC_MASK,
                self.PG_MASK,
                self.VAC2_PRESENT_MASK,
                self.VAC1_PRESENT_MASK,
                self.VBUS_PRESENT_MASK,
            )
        ...
        def get_IINDPM_MASK_string(self):
            ...
            1h = Enter IINDPM / IOTG does NOT produce INT pulse
            """
            if self.IINDPM_MASK == 0:
                return "Enter IINDPM / IOTG does produce INT pulse"
            elif self.IINDPM_MASK == 1:
                return "Enter IINDPM / IOTG does NOT produce INT pulse"
            else:
                return "unknown"
        ...
        def get_VINDPM_MASK_string(self):
            ...
            1h = Enter VINDPM / VOTG does NOT produce INT pulse
            """
            if self.VINDPM_MASK == 0:
                return "Enter VINDPM / VOTG does produce INT pulse"
            elif self.VINDPM_MASK == 1:
                return "Enter VINDPM / VOTG does NOT produce INT pulse"
            else:
                return "unknown"
        ...
        def get_WD_MASK_string(self):
            ...
            """
            if self.WD_MASK == 0:
                return "I2C watch dog timer expired does produce INT pulse"
            elif self.WD_MASK == 1:
                return "I2C watch dog timer expired does NOT produce INT pulse"
            else:
                return "unknown"
        ...
        def get_POORSRC_MASK_string(self):
            ...
            1h = Poor source detected does NOT produce INT
            """
            if self.POORSRC_MASK == 0:
                return "Poor source detected does produce INT"
            elif self.POORSRC_MASK == 1:
                return "Poor source detected does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_PG_MASK_string(self):
            ...
            """
            if self.PG_MASK == 0:
                return "PG toggle does produce INT"
            elif self.PG_MASK == 1:
                return "PG toggle does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VAC2_PRESENT_MASK_string(self):
            ...
            1h = VAC2 present status change does NOT produce INT
            """
            if self.VAC2_PRESENT_MASK == 0:
                return "VAC2 present status change does produce INT"
            elif self.VAC2_PRESENT_MASK == 1:
                return "VAC2 present status change does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VAC1_PRESENT_MASK_string(self):
            ...
            1h = VAC1 present status change does NOT produce INT
            """
            if self.VAC1_PRESENT_MASK == 0:
                return "VAC1 present status change does produce INT"
            elif self.VAC1_PRESENT_MASK == 1:
                return "VAC1 present status change does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VBUS_PRESENT_MASK_string(self):
            ...
            1h = VBUS present status change does NOT produce INT
            """
            if self.VBUS_PRESENT_MASK == 0:
                return "VBUS present status change does produce INT"
            elif self.VBUS_PRESENT_MASK == 1:
                return "VBUS present status change does NOT produce INT"
            else:
                return "unknown"
    ...
    class REG29_Charger_Mask_1(BQ25795_REGISTER):
        ...
        def get(self):
            self._value = (
                ...
                | 0
                | (self.TREG_MASK << 2)
                | (self.VBAT_PRESENT_MASK << 1)
                | (self.BC1_2_DONE_MASK << 0)
            )
            return (
                self._value,
                self.CHG_MASK,
                ...
                self.VBUS_MASK,
                self.TREG_MASK,
                self.VBAT_PRESENT_MASK,
                self.BC1_2_DONE_MASK,
            )
        ...
        def get_CHG_MASK_string(self):
            ...
            """
            if self.CHG_MASK == 0:
                return "Charging status change does produce INT"
            elif self.CHG_MASK == 1:
                return "Charging status change does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_ICO_MASK_string(self):
            ...
            """
            if self.ICO_MASK == 0:
                return "ICO status change does produce INT"
            elif self.ICO_MASK == 1:
                return "ICO status change does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VBUS_MASK_string(self):
            ...
            """
            if self.VBUS_MASK == 0:
                return "VBUS status change does produce INT"
            elif self.VBUS_MASK == 1:
                return "VBUS status change does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_TREG_MASK_string(self):
            ...
            """
            if self.TREG_MASK == 0:
                return "TREG status change does produce INT"
            elif self.TREG_MASK == 1:
                return "TREG status change does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VBAT_PRESENT_MASK_string(self):
            ...
            1h = VBAT present status change does NOT produce INT
            """
            if self.VBAT_PRESENT_MASK == 0:
                return "VBAT present status change does produce INT"
            elif self.VBAT_PRESENT_MASK == 1:
                return "VBAT present status change does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_BC1_2_DONE_MASK_string(self):
            ...
            1h = BC1.2 done status change does NOT produce INT
            """
            if self.BC1_2_DONE_MASK == 0:
                return "BC1.2 done status change does produce INT"
            elif self.BC1_2_DONE_MASK == 1:
                return "BC1.2 done status change does NOT produce INT"
            else:
                return "unknown"
    ...
    class REG2A_Charger_Mask_2(BQ25795_REGISTER):
        ...
        def get(self):
            ...
            self._value = (
                0
                | (self.DPDM_DONE_MASK << 6)
                | (self.ADC_DONE_MASK << 5)
                | (self.VSYS_MASK << 4)
                | (self.CHG_TMR_MASK << 3)
                | (self.TRICHG_TMR_MASK << 2)
                | (self.PRECHG_TMR_MASK << 1)
                | (self.TOPOFF_TMR_MASK << 0)
            )
            return (
                self._value,
                self.DPDM_DONE_MASK,
                ...
                self.VSYS_MASK,
                self.CHG_TMR_MASK,
                self.TRICHG_TMR_MASK,
                self.PRECHG_TMR_MASK,
                self.TOPOFF_TMR_MASK,
            )
        ...
        def get_DPDM_DONE_MASK_string(self):
            ...
            1h = D+/D- detection done does NOT produce INT pulse
            """
            if self.DPDM_DONE_MASK == 0:
                return "D+/D- detection done does produce INT pulse"
            elif self.DPDM_DONE_MASK == 1:
                return "D+/D- detection done does NOT produce INT pulse"
            else:
                return "unknown"
        ...
        def get_ADC_DONE_MASK_string(self):
            ...
            1h = ADC conversion done does NOT produce INT pulse
            """
            if self.ADC_DONE_MASK == 0:
                return "ADC conversion done does produce INT pulse"
            elif self.ADC_DONE_MASK == 1:
                return "ADC conversion done does NOT produce INT pulse"
            else:
                return "unknown"
        ...
        def get_VSYS_MASK_string(self):
            ...
            """
            if self.VSYS_MASK == 0:
                return "enter or exit VSYSMIN regulation does produce INT pulse"
            elif self.VSYS_MASK == 1:
                return "enter or exit VSYSMIN regulation does NOT produce INT pulse"
            else:
                return "unknown"
        ...
        def get_CHG_TMR_MASK_string(self):
            ...
            1h = Fast charge timer expire does NOT produce INT
            """
            if self.CHG_TMR_MASK == 0:
                return "Fast charge timer expire does produce INT"
            elif self.CHG_TMR_MASK == 1:
                return "Fast charge timer expire does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_TRICHG_TMR_MASK_string(self):
            ...
            1h = Trickle charge timer expire does NOT produce INT
            """
            if self.TRICHG_TMR_MASK == 0:
                return "Trickle charge timer expire does produce INT"
            elif self.TRICHG_TMR_MASK == 1:
                return "Trickle charge timer expire does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_PRECHG_TMR_MASK_string(self):
            ...
            1h = Precharge timer expire does NOT produce INT
            """
            if self.PRECHG_TMR_MASK == 0:
                return "Precharge timer expire does produce INT"
            elif self.PRECHG_TMR_MASK == 1:
                return "Precharge timer expire does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_TOPOFF_TMR_MASK_string(self):
            ...
            1h = Top off timer expire does NOT produce INT
            """
            if self.TOPOFF_TMR_MASK == 0:
                return "Top off timer expire does produce INT"
            elif self.TOPOFF_TMR_MASK == 1:
                return "Top off timer expire does NOT produce INT"
            else:
                return "unknown"
    ...
    class REG2B_Charger_Mask_3(BQ25795_REGISTER):
        ...
        def get_TS_COLD_MASK_string(self):
            ...
            1h = TS across cold temperature (T1) does NOT produce INT
            """
            if self.TS_COLD_MASK == 0:
                return "TS across cold temperature (T1) does produce INT"
            elif self.TS_COLD_MASK == 1:
                return "TS across cold temperature (T1) does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_TS_COOL_MASK_string(self):
            ...
            1h = TS across cool temperature (T2) does NOT produce INT
            """
            if self.TS_COOL_MASK == 0:
                return "TS across cool temperature (T2) does produce INT"
            elif self.TS_COOL_MASK == 1:
                return "TS across cool temperature (T2) does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_TS_WARM_MASK_string(self):
            ...
            1h = TS across warm temperature (T3) does NOT produce INT
            """
            if self.TS_WARM_MASK == 0:
                return "TS across warm temperature (T3) does produce INT"
            elif self.TS_WARM_MASK == 1:
                return "TS across warm temperature (T3) does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_TS_HOT_MASK_string(self):
            ...
            1h = TS across hot temperature (T5) does NOT produce INT
            """
            if self.TS_HOT_MASK == 0:
                return "TS across hot temperature (T5) does produce INT"
            elif self.TS_HOT_MASK == 1:
                return "TS across hot temperature (T5) does NOT produce INT"
            else:
                return "unknown"
    ...
    class REG2C_FAULT_Mask_0(BQ25795_REGISTER):
        ...
        def get(self):
            ...
            self._value = (
                0
                | (self.IBAT_REG_MASK << 7)
                | (self.VBUS_OVP_MASK << 6)
                | (self.VBAT_OVP_MASK << 5)
                | (self.IBUS_OCP_MASK << 4)
                | (self.IBAT_OCP_MASK << 3)
                | (self.CONV_OCP_MASK << 2)
                | (self.VAC2_OVP_MASK << 1)
                | (self.VAC1_OVP_MASK << 0)
            )
            return (
                self._value,
                self.IBAT_REG_MASK,
        ...
        def get_IBAT_REG_MASK_string(self):
            ...
            1h = enter or exit IBAT regulation does NOT produce INT
            """
            if self.IBAT_REG_MASK == 0:
                return "enter or exit IBAT regulation does produce INT"
            elif self.IBAT_REG_MASK == 1:
                return "enter or exit IBAT regulation does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VBUS_OVP_MASK_string(self):
            ...
            1h = entering VBUS OVP does NOT produce INT
            """
            if self.VBUS_OVP_MASK == 0:
                return "entering VBUS OVP does produce INT"
            elif self.VBUS_OVP_MASK == 1:
                return "entering VBUS OVP does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VBAT_OVP_MASK_string(self):
            ...
            1h = entering VBAT OVP does NOT produce INT
            """
            if self.VBAT_OVP_MASK == 0:
                return "entering VBAT OVP does produce INT"
            elif self.VBAT_OVP_MASK == 1:
                return "entering VBAT OVP does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_IBUS_OCP_MASK_string(self):
            ...
            1h = entering IBUS OCP does NOT produce INT
            """
            if self.IBUS_OCP_MASK == 0:
                return "entering IBUS OCP does produce INT"
            elif self.IBUS_OCP_MASK == 1:
                return "entering IBUS OCP does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_IBAT_OCP_MASK_string(self):
            ...
            1h = entering IBAT OCP does NOT produce INT
            """
            if self.IBAT_OCP_MASK == 0:
                return "entering IBAT OCP does produce INT"
            elif self.IBAT_OCP_MASK == 1:
                return "entering IBAT OCP does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_CONV_OCP_MASK_string(self):
            ...
            1h = entering converter OCP does NOT produce INT
            """
            if self.CONV_OCP_MASK == 0:
                return "entering converter OCP does produce INT"
            elif self.CONV_OCP_MASK == 1:
                return "entering converter OCP does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VAC2_OVP_MASK_string(self):
            ...
            1h = entering VAC2 OVP does NOT produce INT
            """
            if self.VAC2_OVP_MASK == 0:
                return "entering VAC2 OVP does produce INT"
            elif self.VAC2_OVP_MASK == 1:
                return "entering VAC2 OVP does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VAC1_OVP_MASK_string(self):
            ...
            1h = entering VAC1 OVP does NOT produce INT
            """
            if self.VAC1_OVP_MASK == 0:
                return "entering VAC1 OVP does produce INT"
            elif self.VAC1_OVP_MASK == 1:
                return "entering VAC1 OVP does NOT produce INT"
            else:
                return "unknown"
    ...
    class REG2D_FAULT_Mask_1(BQ25795_REGISTER):
        ...
        def get(self):
            ...
            self._value = (
                0
                | (self.VSYS_SHORT_MASK << 7)
                | (self.VSYS_OVP_MASK << 6)
                | (self.OTG_OVP_MASK << 5)
                | (self.OTG_UVP_MASK << 4)
                | (self.TSHUT_MASK << 2)
            )
        ...
        def get_VSYS_SHORT_MASK_string(self):
            ...
            1h = entering VSYS short circuit does NOT produce INT
            """
            if self.VSYS_SHORT_MASK == 0:
                return "entering VSYS short circuit does produce INT"
            elif self.VSYS_SHORT_MASK == 1:
                return "entering VSYS short circuit does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_VSYS_OVP_MASK_string(self):
            ...
            1h = entering VSYS OVP does NOT produce INT
            """
            if self.VSYS_OVP_MASK == 0:
                return "entering VSYS OVP does produce INT"
            elif self.VSYS_OVP_MASK == 1:
                return "entering VSYS OVP does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_OTG_OVP_MASK_string(self):
            ...
            1h = OTG VBUS over-voltage fault does NOT produce INT
            """
            if self.OTG_OVP_MASK == 0:
                return "OTG VBUS over-voltage fault does produce INT"
            elif self.OTG_OVP_MASK == 1:
                return "OTG VBUS over-voltage fault does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_OTG_UVP_MASK_string(self):
            ...
            1h = OTG VBUS under-voltage fault does NOT produce INT
            """
            if self.OTG_UVP_MASK == 0:
                return "OTG VBUS under-voltage fault does produce INT"
            elif self.OTG_UVP_MASK == 1:
                return "OTG VBUS under-voltage fault does NOT produce INT"
            else:
                return "unknown"
        ...
        def get_TSHUT_MASK_string(self):
            ...
            1h = entering thermal shutdown does NOT produce INT
            """
            if self.TSHUT_MASK == 0:
                return "entering thermal shutdown does produce INT"
            elif self.TSHUT_MASK == 1:
                return "entering thermal shutdown does NOT produce INT"
            else:
                return "unknown"
    ...
    class REG2F_ADC_Function_Disable_0(BQ25795_REGISTER):
        ...
        def get_IBUS_ADC_DIS_string(self):
            ...
            1h = Disable IBUS ADC
            """
            if self.IBUS_ADC_DIS == 0:
                return "Enable IBUS ADC"
            elif self.IBUS_ADC_DIS == 1:
                return "Disable IBUS ADC"
            else:
                return "unknown"
        ...
        def get_IBAT_ADC_DIS_string(self):
            ...
            1h = Disable IBAT ADC
            """
            if self.IBAT_ADC_DIS == 0:
                return "Enable IBAT ADC"
            elif self.IBAT_ADC_DIS == 1:
                return "Disable IBAT ADC"
            else:
                return "unknown"
        ...
        def get_VBUS_ADC_DIS_string(self):
            ...
            1h = Disable VBUS ADC
            """
            if self.VBUS_ADC_DIS == 0:
                return "Enable VBUS ADC"
            elif self.VBUS_ADC_DIS == 1:
                return "Disable VBUS ADC"
            else:
                return "unknown"
        ...
        def get_VBAT_ADC_DIS_string(self):
            ...
            1h = Disable VBAT ADC
            """
            if self.VBAT_ADC_DIS == 0:
                return "Enable VBAT ADC"
            elif self.VBAT_ADC_DIS == 1:
                return "Disable VBAT ADC"
            else:
                return "unknown"
        ...
        def get_VSYS_ADC_DIS_string(self):
            ...
            1h = Disable VSYS ADC
            """
            if self.VSYS_ADC_DIS == 0:
                return "Enable VSYS ADC"
            elif self.VSYS_ADC_DIS == 1:
                return "Disable VSYS ADC"
            else:
                return "unknown"
        ...
        def get_TS_ADC_DIS_string(self):
            ...
            1h = Disable TS ADC
            """
            if self.TS_ADC_DIS == 0:
                return "Enable TS ADC"
            elif self.TS_ADC_DIS == 1:
                return "Disable TS ADC"
            else:
                return "unknown"
        ...
        def get_TDIE_ADC_DIS_string(self):
            ...
            1h = Disable TDIE ADC
            """
            if self.TDIE_ADC_DIS == 0:
                return "Enable TDIE ADC"
            elif self.TDIE_ADC_DIS == 1:
                return "Disable TDIE ADC"
            else:
                return "unknown"
    ...
    class REG30_ADC_Function_Disable_1(BQ25795_REGISTER):
        ...
        def get_DP_ADC_DIS_string(self):
            ...
            1h = Disable
            """
            if self.DP_ADC_DIS == 0:
                return "Enable (Default)"
            elif self.DP_ADC_DIS == 1:
                return "Disable"
            else:
                return "unknown"
        ...
        def get_DM_ADC_DIS_string(self):
            ...
            1h = Disable
            """
            if self.DM_ADC_DIS == 0:
                return "Enable (Default)"
            elif self.DM_ADC_DIS == 1:
                return "Disable"
            else:
                return "unknown"
        ...
        def get_VAC2_ADC_DIS_string(self):
            ...
            1h = Disable VAC2 ADC
            """
            if self.VAC2_ADC_DIS == 0:
                return "Enable VAC2 ADC"
            elif self.VAC2_ADC_DIS == 1:
                return "Disable VAC2 ADC"
            else:
                return "unknown"
        ...
        def get_VAC1_ADC_DIS_string(self):
            ...
            1h = Disable VAC1 ADC
            """
            if self.VAC1_ADC_DIS == 0:
                return "Enable VAC1 ADC"
            elif self.VAC1_ADC_DIS == 1:
                return "Disable VAC1 ADC"
            else:
                return "unknown"
    ...
    class REG47_DPDM_Driver(BQ25795_REGISTER):
        ...
        def get_DPLUS_DAC_string(self):
            ...
            if self.DPLUS_DAC == 0:
                return "HIZ"
            elif self.DPLUS_DAC == 1:
                return "0V"
            elif self.DPLUS_DAC == 2:
                return "0.6V"
            elif self.DPLUS_DAC == 3:
                return "1.2V"
            elif self.DPLUS_DAC == 4:
                return "2.0V"
            elif self.DPLUS_DAC == 5:
                return "2.7V"
            elif self.DPLUS_DAC == 6:
                return "3.3V"
            elif self.DPLUS_DAC == 7:
                return "D+/D- Short"
            else:
                return "unknown"
        ...
        def get_DMINUS_DAC_string(self):
            ...
            Returns the D- driver string based on the current value.
            """
            if self.DMINUS_DAC == 0:
                return "HIZ"
            elif self.DMINUS_DAC == 1:
                return "0V"
            elif self.DMINUS_DAC == 2:
                return "0.6V"
            elif self.DMINUS_DAC == 3:
                return "1.2V"
            elif self.DMINUS_DAC == 4:
                return "2.0V"
            elif self.DMINUS_DAC == 5:
                return "2.7V"
            elif self.DMINUS_DAC == 6:
                return "3.3V"
            elif self.DMINUS_DAC == 7:
                return "reserved"
            else:
                return "unknown"
    ...
    class REG48_Part_Information(BQ25795_REGISTER):
        ...
        def get_PART_REVISION_string(self):
            ...
            if self.PART_REVISION == 0:
                return "BQ25792"
            elif self.PART_REVISION == 1:
                return "Reserved"
            elif self.PART_REVISION == 2:
                return "Reserved"
            elif self.PART_REVISION == 3:
                return "Reserved"
            elif self.PART_REVISION == 4:
                return "Reserved"
            elif self.PART_REVISION == 5:
                return "Reserved"
            elif self.PART_REVISION == 6:
                return "Reserved"
            elif self.PART_REVISION == 7:
                return "Reserved"
            else:
                return "unknown"
    ...
    def read_register(self, reg_addr, length=1):
        ...
        return self.safe_execute(
            self.bq.read_i2c_block_data, self.i2c_addr, reg_addr, length
        )
    ...
    def write_register_word(self, reg):
        ...
        reg.get()
        self.safe_execute(
            self.bq.write_byte_data, self.i2c_addr, reg._addr, reg._value & 0xFF
        )
        self.safe_execute(
            self.bq.write_byte_data,
            self.i2c_addr,
            reg._addr + 1,
            (reg._value >> 8) & 0xFF,
        )
    ...
    def read_all_register(self):
        ...
        try:
            ...
            self.REG00_Minimal_System_Voltage.set(
                self.registers[self.REG00_Minimal_System_Voltage._addr]
            )
            self.REG01_Charge_Voltage_Limit.set(
                (self.registers[self.REG01_Charge_Voltage_Limit._addr] << 8)
                | (self.registers[self.REG01_Charge_Voltage_Limit._addr + 1])
            )
            self.REG03_Charge_Current_Limit.set(
                (self.registers[self.REG03_Charge_Current_Limit._addr] << 8)
                | (self.registers[self.REG03_Charge_Current_Limit._addr + 1])
            )
            self.REG05_Input_Voltage_Limit.set(
                self.registers[self.REG05_Input_Voltage_Limit._addr]
            )
            self.REG06_Input_Current_Limit.set(
                (self.registers[self.REG06_Input_Current_Limit._addr] << 8)
                | (self.registers[self.REG06_Input_Current_Limit._addr + 1])
            )
            self.REG08_Precharge_Control.set(
                self.registers[self.REG08_Precharge_Control._addr]
            )
            self.REG09_Termination_Control.set(
                self.registers[self.REG09_Termination_Control._addr]
            )
            self.REG0A_Recharge_Control.set(
                self.registers[self.REG0A_Recharge_Control._addr]
            )
            self.REG0B_VOTG_regulation.set(
                (self.registers[self.REG0B_VOTG_regulation._addr] << 8)
                | (self.registers[self.REG0B_VOTG_regulation._addr + 1])
            )
            self.REG0D_IOTG_regulation.set(
                self.registers[self.REG0D_IOTG_regulation._addr]
            )
            self.REG0E_Timer_Control.set(self.registers[self.REG0E_Timer_Control._addr])
            self.REG0F_Charger_Control_0.set(
                self.registers[self.REG0F_Charger_Control_0._addr]
            )
            self.REG10_Charger_Control_1.set(
                self.registers[self.REG10_Charger_Control_1._addr]
            )
            self.REG11_Charger_Control_2.set(
                self.registers[self.REG11_Charger_Control_2._addr]
            )
            self.REG12_Charger_Control_3.set(
                self.registers[self.REG12_Charger_Control_3._addr]
            )
            self.REG13_Charger_Control_4.set(
                self.registers[self.REG13_Charger_Control_4._addr]
            )
            self.REG14_Charger_Control_5.set(
                self.registers[self.REG14_Charger_Control_5._addr]
            )
            self.REG15_Reserved.set(self.registers[self.REG15_Reserved._addr])
            self.REG16_Temperature_Control.set(
                self.registers[self.REG16_Temperature_Control._addr]
            )
            ...
            self.REG19_ICO_Current_Limit.set(
                (self.registers[self.REG19_ICO_Current_Limit._addr] << 8)
                | (self.registers[self.REG19_ICO_Current_Limit._addr + 1])
            )
            self.REG1B_Charger_Status_0.set(
                self.registers[self.REG1B_Charger_Status_0._addr]
            )
            self.REG1C_Charger_Status_1.set(
                self.registers[self.REG1C_Charger_Status_1._addr]
            )
            self.REG1D_Charger_Status_2.set(
                self.registers[self.REG1D_Charger_Status_2._addr]
            )
            self.REG1E_Charger_Status_3.set(
                self.registers[self.REG1E_Charger_Status_3._addr]
            )
            self.REG1F_Charger_Status_4.set(
                self.registers[self.REG1F_Charger_Status_4._addr]
            )
            self.REG20_FAULT_Status_0.set(
                self.registers[self.REG20_FAULT_Status_0._addr]
            )
            self.REG21_FAULT_Status_1.set(
                self.registers[self.REG21_FAULT_Status_1._addr]
            )
            self.REG22_Charger_Flag_0.set(
                self.registers[self.REG22_Charger_Flag_0._addr]
            )
            self.REG23_Charger_Flag_1.set(
                self.registers[self.REG23_Charger_Flag_1._addr]
            )
            self.REG24_Charger_Flag_2.set(
                self.registers[self.REG24_Charger_Flag_2._addr]
            )
            self.REG25_Charger_Flag_3.set(
                self.registers[self.REG25_Charger_Flag_3._addr]
            )
            ...
            self.REG28_Charger_Mask_0.set(
                self.registers[self.REG28_Charger_Mask_0._addr]
            )
            self.REG29_Charger_Mask_1.set(
                self.registers[self.REG29_Charger_Mask_1._addr]
            )
            self.REG2A_Charger_Mask_2.set(
                self.registers[self.REG2A_Charger_Mask_2._addr]
            )
            self.REG2B_Charger_Mask_3.set(
                self.registers[self.REG2B_Charger_Mask_3._addr]
            )
            ...
            self.REG2F_ADC_Function_Disable_0.set(
                self.registers[self.REG2F_ADC_Function_Disable_0._addr]
            )
            self.REG30_ADC_Function_Disable_1.set(
                self.registers[self.REG30_ADC_Function_Disable_1._addr]
            )
            self.REG31_IBUS_ADC.set(
                (self.registers[self.REG31_IBUS_ADC._addr] << 8)
                | (self.registers[self.REG31_IBUS_ADC._addr + 1])
            )
            self.REG33_IBAT_ADC.set(
                (self.registers[self.REG33_IBAT_ADC._addr] << 8)
                | (self.registers[self.REG33_IBAT_ADC._addr + 1])
            )
            self.REG35_VBUS_ADC.set(
                (self.registers[self.REG35_VBUS_ADC._addr] << 8)
                | (self.registers[self.REG35_VBUS_ADC._addr + 1])
            )
            self.REG37_VAC1_ADC.set(
                (self.registers[self.REG37_VAC1_ADC._addr] << 8)
                | (self.registers[self.REG37_VAC1_ADC._addr + 1])
            )
            self.REG39_VAC2_ADC.set(
                (self.registers[self.REG39_VAC2_ADC._addr] << 8)
                | (self.registers[self.REG39_VAC2_ADC._addr + 1])
            )
            self.REG3B_VBAT_ADC.set(
                (self.registers[self.REG3B_VBAT_ADC._addr] << 8)
                | (self.registers[self.REG3B_VBAT_ADC._addr + 1])
            )
            self.REG3D_VSYS_ADC.set(
                (self.registers[self.REG3D_VSYS_ADC._addr] << 8)
                | (self.registers[self.REG3D_VSYS_ADC._addr + 1])
            )
            self.REG3F_TS_ADC.set(
                (self.registers[self.REG3F_TS_ADC._addr] << 8)
                | (self.registers[self.REG3F_TS_ADC._addr + 1])
            )
            self.REG41_TDIE_ADC.set(
                (self.registers[self.REG41_TDIE_ADC._addr] << 8)
                | (self.registers[self.REG41_TDIE_ADC._addr + 1])
            )
            self.REG43_DP_ADC.set(
                (self.registers[self.REG43_DP_ADC._addr] << 8)
                | (self.registers[self.REG43_DP_ADC._addr + 1])
            )
            self.REG45_DM_ADC.set(
                (self.registers[self.REG45_DM_ADC._addr] << 8)
                | (self.registers[self.REG45_DM_ADC._addr + 1])
            )
            self.REG47_DPDM_Driver.set(self.registers[self.REG47_DPDM_Driver._addr])
            self.REG48_Part_Information.set(
                self.registers[self.REG48_Part_Information._addr]
            )
            return 0
        except I2CError:
    ...
    def set_input_current_limit(self, input_current_limit: int) -> None:
        ...
        try:
            ...
            if input_current_limit < 100 or input_current_limit > 3300:
                raise ValueError(
                    "Input current limit must be between 100mA and 3300mA (inclusive)."
                )
    ...
    def bq25792_REG1C_Charger_Status_1(self, i2c_read=True):
        ...
        if CHG_STAT == 0x0:
            ...
            ret = "Top-off Timer Active Charging"
        elif CHG_STAT == 0x07:
            ret = "Charge Termination Done"
        else:
            ret = CHG_STAT
        return val, ret
    ...
    def to_dict(self):
        """
        Returns dictionary with all charger data.
        ...
        charger_data = ChargerData(
            ...
            battery_connected=self.read_VBAT_PRESENT(),
            bat_soc=bat_SOC,
            bat_stat=bat_Stat,
