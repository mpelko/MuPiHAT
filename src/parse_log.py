...
figure_file = re.search(r"(.*)\..+$", filename).group(1) + ".png"
print(figure_file)
...
while True:
    ...
    if x:
        ...
        # VBAT?
        x = re.search(r"(     Battery Voltage  \[mV\]     =)(.+)", line)
        if x:
            # add VBAT to list
            # tbd
            ...
            # IBAT?
            x = re.search(r"(     IBAT \(Dis-\)Charge \[mA\]    =)(.+)", line)
            if x:
                # add IBAT to list
                # tbd
...
for i in range(1, len(timestamp)):
    ...
    Qmax = Qmax + (
        (ibat[i - 1] / 1000)
        * (vbat[i - 1] / 1000)
        * pd.to_numeric(dt.seconds / 60 / 60)
    )
...
fig = plt.figure()
ax = fig.add_subplot()
ax.text(80, 5000, f"Estimated Capacity [Wh]: {Qmax:.2f} \nDuration {test_duration}")
