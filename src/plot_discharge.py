...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Plot battery voltage vs. state of charge from a log file"
    )
    parser.add_argument(
        "-l", "--logfile", required=True, help="Path to the log file to be analyzed"
    )
    parser.add_argument(
        "-o",
        "--output",
...
def main():
    ...
    if (
        len(timestamps) > len(ibat_values)
        or len(timestamps) > len(vbat_values)
        or len(timestamps) > len(charger_statuses)
    ):
    ...
    timestamps = [
        ...
        for ts, status in zip(timestamps, charger_statuses)
    ]
    ...
    for i in range(1, len(timestamps)):
        ...
        Qmax += (
            (ibat_values[i - 1] / 1000)
            * (vbat_values[i - 1] / 1000)
            * pd.to_numeric(dt.seconds / 3600)
        )
    ...
    for i, vbat in zip(x_labels, vbat_at_x):
        plt.text(
            ...
            color="black",
            ha="center",
            bbox=dict(facecolor="lightgrey", alpha=0.8, boxstyle="round,pad=0.3"),
            fontsize=8,
            fontweight="bold",
        )
