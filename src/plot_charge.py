...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Plot battery current and voltage from a log file."
    )
    parser.add_argument(
        "-l", "--logfile", type=str, help="Path to the log file", required=True
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        help="Output file path (including file extension)",
        required=True,
    )
